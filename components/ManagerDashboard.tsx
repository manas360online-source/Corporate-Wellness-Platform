    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={TEAM_WELLNESS_DATA} layout="vertical" margin={{ left: 60 }}>
                ...
                <YAxis dataKey="metric" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#94a3b8', fontWeight: 900, textTransform: 'uppercase' }} />
        ...
        <div className="bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-xl shadow-blue-500/5">
          ...
          <div className="mt-10 p-8 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl">
            <p className="text-[10px] font-black mb-2 uppercase tracking-[0.25em] text-[#1d7dfa]">Privacy Integrity Shield</p>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed opacity-80">MANAS360 utilizes k-anonymity protocols. Individual data trajectories are mathematically shielded to ensure absolute employee anonymity.</p>
          </div>
        </div>
      </div>
    </div>
  ...
  const renderAnalytics = () => (
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="bg-[#1d7dfa] rounded-[3.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
           <h4 className="text-2xl font-black mb-4 tracking-tight">Decision Support AI</h4>
           <p className="text-white/80 font-bold text-sm uppercase leading-relaxed tracking-widest group-hover:text-white transition-colors">
           ...
           <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-white/10 blur-[60px]"></div>
        </div>
      </div>
    </div>
  );
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={TEAM_RANKINGS} margin={{ top: 20 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f8fafc" />
                <XAxis dataKey="dept" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#94a3b8', fontWeight: 900, textTransform: 'uppercase' }} />
            ...
            {ANIMAL_NAMES.slice(0, 5).map((animal, i) => (
              <div key={animal} className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-sm font-black text-slate-700 tracking-tight">Anonymous {animal}</span>
                </div>
