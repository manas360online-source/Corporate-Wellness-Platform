    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          ...
          className="px-10 py-5 bg-[#1d7dfa] text-white rounded-full font-black text-xs uppercase tracking-[0.25em] shadow-xl shadow-blue-200 hover:bg-blue-600 transition-all active:scale-95 flex items-center space-x-3"
        >
          <span>➕</span>
          ...
          <div key={challenge.id} className="bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-xl shadow-blue-500/5 group hover:shadow-2xl hover:border-blue-100 transition-all">
            <div className="flex flex-col md:flex-row gap-10 items-center">
              ...
              <div className="flex items-center space-x-12 shrink-0">
                ...
                <button 
                  onClick={() => alert('Access Control: Modify permissions limited to Global Admins.')}
            ...
            {!scheduled ? (
              <>
                <div className="max-w-xl">
                  <h4 className="text-3xl font-black mb-4 tracking-tighter">Bespoke Enterprise Solutions</h4>
                ...
                <button 
                  ...
                  disabled={submitting}
                  className="px-12 py-6 bg-[#1d7dfa] hover:bg-blue-600 disabled:bg-slate-800 transition-all rounded-full font-black text-xs uppercase tracking-[0.25em] shadow-2xl shadow-blue-500/20 active:scale-95 flex items-center space-x-3 shrink-0"
                >
                  {submitting ? (
                    <>
              ...
              <div className="flex flex-col items-center md:items-start space-y-4 animate-in slide-in-from-right-10 duration-500 w-full">
                ...
                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">A MANAS360 Strategy Architect will synchronize with <strong className="text-white">priya.rao@ibm.com</strong> within 1 business cycle.</p>
                <button onClick={() => setScheduled(false)} className="text-[10px] font-black text-[#1d7dfa] uppercase tracking-[0.2em] underline mt-6 hover:text-white transition-colors">Revoke Request</button>
              </div>
            )}
         </div>
         <div className="absolute top-0 right-0 w-80 h-80 bg-blue-600/10 blur-[80px] -mr-20 -mt-20"></div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-[3.5rem] p-12 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="space-y-8">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Protocol Identity</label>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Modality</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Cycle Duration (Days)</label>
